...
export function extractContours(source, boundingBox, threshold = 50) {
  ...
  try {
    ...
    const x = Math.max(0, Math.min(boundingBox.x, srcWidth));
    const y = Math.max(0, Math.min(boundingBox.y, srcHeight));
    const width = Math.min(boundingBox.width, srcWidth - x);
    const height = Math.min(boundingBox.height, srcHeight - y);
...
/**
 * Draw contour outline on canvas
 ...
 * Uses a stylized rectangle-based outline with rounded corners
 * @param {CanvasRenderingContext2D} ctx - Canvas context  
...
export function drawFallbackOutline(ctx, boundingBox, color, lineWidth = 2) {
  ...
  const radius = Math.min(width, height) * 0.1; // 10% corner radius
